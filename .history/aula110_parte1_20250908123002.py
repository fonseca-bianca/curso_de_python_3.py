...
Ref.: https://www.youtube.com/watch?v=EL-4sQ9v2dc

- no cód abaixo, os itens foram AGRUPADOS e não ORDENADOS 
    OBS.: caso o interesse fosse pra ORDENAR os itens, aí seria prudente usar 
    a função sorted()


PROBLEMA GERADO (abaixo):
...
grouped_by_department = {} # dict organiza nomes
for name, department in employees:
# se o departamento já existir no dict, o nome do funcionário é adc à lista 
# de nomes daquele departamento
    if department in grouped_by_department:
        grouped_by_department[department].append(name)
    else:
# Se o departamento ainda não existir, ele é criado como uma NOVA chave no 
# dict e o nome do funcionário é adc a uma NOVA lista associada a essa chave
        grouped_by_department[department] = [name]
for department, names in grouped_by_department.items():
    print(department, names)
    
# output:
# IT ['Vera', 'Chuck', 'Karl', 'Jack']
# ADMIN ['Dave', 'Janet']