...
count(): 
    ...
    * range(): NÃO é um iterator, MAS é iterável
    ...
    limite         | um 'break'
    pode passar    | só pode passar 
    início e fim   | início
    
    ** Ambos:
    recebem step
    argumentos nomeados/keyword arguments: palavra-chave do parâmetro ao qual
    está sendo atribuído um valor na chamada da função
    ex.: c1 = count(step=8, start=0)
...
# Passando início pro count() e início e fim pro range():
c1 = count(10)
r1 = range(10, 100)
...
for i in c1:
    if i >= 100:
        break
    print(i)
    # vai imprimir de 10 até 100, COM condição de parada

print("For no 'range':")
for j in r1:
    print(j)
    # vai imprimir de 10 até 100, SEM condição de parada