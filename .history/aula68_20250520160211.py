...
def funcao_externa():
    def funcao_interna():
        ...
        print("Dentro da função interna:", a, b)  # Acessa 'a' (global) e 'b' (local)
    
    funcao_interna()
    print("Dentro da função externa:", a)  # Acessa apenas 'a', pois 'b' está fora do seu escopo

funcao_externa()

# OBS: A função externa não acessa 'b' da interna, mas a interna acessa 'a' da externa/global
...
def escopo_externo():
    ...
    def escopo_interno():
        ...
        print("Função interna (x e y):", x, y)  # Acessa x=10 (do escopo externo) e y=2 (local)
