...
while True:
    user_command = input("Digite um comando (listar, desfazer, refazer): ")
    ...
    elif user_command == 'desfazer':
        undo = []
        
        task_undo = to_do_list.pop()
        undo.append(task_undo)
        print(to_do_list)
        continue

    elif user_command == 'refazer':
           
        task_redone = undo.pop()
        to_do_list.append(task_redone)
        print(f"Tarefa '{task_redone}' refeita.")
        list_tasks()
        continue
