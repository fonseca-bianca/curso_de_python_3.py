...
resultado_todos_digitos = 0
for digito in soma_nove_digitos:
    multiplicacao_digitos = int(digito) * contagem_regressiva # int: converte a string pra inteiro
    print(digito, contagem_regressiva, multiplicacao_digitos) # aq vai mostrar cada dígito do CPF e seu equivalente na contagem regressiva
    resultado_todos_digitos += multiplicacao_digitos
    contagem_regressiva -= 1 # mostra a contagem regressiva

print(f"A soma de todos os números multiplicados é: {resultado_todos_digitos}")

obter_digito = ((resultado_todos_digitos * 10) % 11)
obter_digito = obter_digito if obter_digito <= 9 else 0
print(obter_digito)
